...
- Automatically restore remembered encodings for both working-tree files and VS Code Git diff documents.
...
## Git Diff for Non-UTF-8 Files

When VS Code opens a Git diff, the committed side uses a virtual `git:` document rather than the local `file:` URI. Encoding Keeper maps that Git URI back to the corresponding workspace file, reads its encoding record, and automatically reopens the Git document with the same encoding.

After recording GB2312/GBK for a file, you can use the normal Source Control diff. No separate Encoding Keeper compare command is required. This is equivalent to manually running VS Code's "Reopen with Encoding" action on the Git side of the diff.
...
```bash
code --install-extension encoding-keeper-0.0.8.vsix
```
